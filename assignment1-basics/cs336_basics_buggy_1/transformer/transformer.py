...
from cs336_basics_buggy_1.transformer.attention import CausalMultiHeadAttention
from cs336_basics_buggy_1.transformer.core import Embedding, Linear, RMSNorm, SwiGLU
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        x = self.ln2(x)
        x = self.ffn(x) # BUG: add x_orig for residual connection
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.ln_final = RMSNorm(d_model, device=device, dtype=dtype)  # BUG: should add eps here
...
if __name__ == "__main__":
    ...
    model = Transformer(d_model=128, num_heads=2, d_ff=512, theta=1e5, vocab_size=vocab_size, context_length=32, num_layers=8)
    ...
    print(f"Logits range: ({output.min()}, {output.max()})")
